...
First Run makes a short list of launch routes from files and package scripts. If the first launch fails and another detected route exists, it tries one alternative without requiring a model key. A transient network error during dependency installation can trigger one retry of the same install command. An optional OpenAI decision can select one of those allowed retries, diagnose a failure even when no alternative launch exists, identify required user input, or stop with a blocker. If the model is unavailable, the tool falls back to its bounded deterministic choices. It never executes a model-supplied shell command.

Enter an OpenAI key in the optional masked field to enable model decisions for that run, or set `OPENAI_API_KEY` before starting First Run. The field is not saved. `FIRST_RUN_MODEL` can override the default `gpt-5-mini`. The provider key is removed from the environment passed to Git and project commands. The failure excerpt, detected project facts, and allowed routes are sent to the model for the decision; review application logs before enabling it for projects containing sensitive output. Setup, verification, and deterministic recovery do not need an API key.
