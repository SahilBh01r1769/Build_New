...
**Try recovery** selects a second bundled Flask fixture. Its `app.py` is a plausible but wrong entry point; `main.py` contains the runnable app. First Run should observe the failed guess and try `main.py`. This fixture checks the recovery path, not broad compatibility. With a valid OpenAI API key entered locally, click **Test key** first, then **Try recovery** and **Set up and run**; the connection status and recovery decision source tell you whether the model participated. **Start again** intentionally uses the saved successful route and does not recreate the failure. The key is optional and should never be pasted into an issue or chat.
