...
An optional OpenAI decision can choose an allowed retry, explain a failure, request input, or stop. The program validates the selected action and route again before executing it. Without the model, rules choose among the same routes. The model cannot supply a shell command or edit source files. Failure labels cover a few recognizable cases; unfamiliar output is shown as an unclear failure.
